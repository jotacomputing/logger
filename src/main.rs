use logger::{logger::{log_flusher::LogFlusher, types::{BalanceLogWrapper, HoldingLogWrapper, OrderLogWrapper, TradeLogs}}, shm::{balance_logs::BalanceLogQueue, holdings_logs::HoldingLogQueue, order_logs::OrderLogQueue, poller::LogPoller}};
...
fn main(){
    ...
    let poller_handle = std::thread::spawn(move||{
       ...
       let mut poller = LogPoller::new(order_log_sender, balance_log_sender, holding_log_sender , trade_log_sender);
       poller.run_poller();
    });

    let flusher_handle = std::thread::spawn(move ||{
        core_affinity::set_for_current(core_affinity::CoreId { id: 4 });
        let mut flusher = LogFlusher::new(order_log_receiver, balance_log_receiver, holding_log_receiver , trade_log_receiver);
        flusher.run();
    });
